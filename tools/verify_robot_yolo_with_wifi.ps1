...
$OutLog = Join-Path $LogDir 'verify_robot_yolo.out.log'
$ErrLog = Join-Path $LogDir 'verify_robot_yolo.err.log'
...
try {
    ...
    Write-Host 'Verifying robot YOLO online...'
    $proc = Start-Process -FilePath python -ArgumentList @('tools/verify_robot_yolo.py') `

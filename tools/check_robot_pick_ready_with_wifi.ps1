...
$OutLog = Join-Path $LogDir 'check_robot_pick_ready.out.log'
$ErrLog = Join-Path $LogDir 'check_robot_pick_ready.err.log'
...
try {
    ...
    $proc = Start-Process -FilePath python -ArgumentList @('tools/check_robot_pick_ready.py') `
    ...
    Write-Host "Check exit code: $($proc.ExitCode)"
    if (Test-Path $OutLog) { Get-Content $OutLog | Out-Host }
    if (Test-Path $ErrLog) { Get-Content $ErrLog | Out-Host }
    if ($proc.ExitCode -ne 0) {
        throw "pick readiness check failed with exit code $($proc.ExitCode)"
    }
}
finally {

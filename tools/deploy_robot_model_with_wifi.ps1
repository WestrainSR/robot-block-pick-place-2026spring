...
$OutLog = Join-Path $LogDir 'deploy_robot_model.out.log'
$ErrLog = Join-Path $LogDir 'deploy_robot_model.err.log'
...
try {
    ...
    Write-Host 'Deploying OpenVINO model to robot...'
    $proc = Start-Process -FilePath python -ArgumentList @('tools/deploy_openvino_to_robot.py') `
    ...
    if ($proc.ExitCode -ne 0) {
        throw "deploy script failed with exit code $($proc.ExitCode)"
    }
}
finally {
